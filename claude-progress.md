...
- **`src/pages/admin.astro`** mounts **`AdminIsland`** with `client:only="react"` (no SSR, no SEO needed). The island reads a `?token=` query param and branches: token present → `MultiImageForm`, otherwise → `MagicLink`. Presence only drives _display_; validity is enforced server-side later by the publish Function (the trust boundary, per ADR 0003).
